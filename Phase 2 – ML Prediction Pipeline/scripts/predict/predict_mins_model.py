#!/usr/bin/env python3
"""
Next-game minutes predictor (team-level next game version)
----------------------------------------------------------
...
• Calculates rolling stats (3/5/10), season averages
• Uses trained Huber model to predict minutes
"""
...
RAW_STATS = ["MIN"]
ADV_STATS = []  # no advanced stats for minutes prediction
...
df_train = pd.read_csv(PROCESSED_TRAINING_MINS)
...
df_schedule["HOME_TEAM_ABV"] = df_schedule["HOME_TEAM_ABV"].astype(str).str.strip().str.upper()
df_schedule["AWAY_TEAM_ABV"] = df_schedule["AWAY_TEAM_ABV"].astype(str).str.strip().str.upper()

print("📥 Loading roster...")
with open(RAW_ROSTERS, "rb") as f:
    roster = pickle.load(f)

print("📥 Loading trained minutes model...")
with open(MINS_HUBER_MODEL, "rb") as f:
    model = pickle.load(f)
...
for _, g in df_schedule.iterrows():
    for team_abv in [g["HOME_TEAM_ABV"], g["AWAY_TEAM_ABV"]]:
...
cat_features = [f for f in model.feature_name_ if f in X.columns and X[f].dtype == "object"]
for c in cat_features:
    X[c] = X[c].astype("category")

df_ghost["MIN_PRED"] = model.predict(X)

# ============================================================
# Save Output
# ============================================================
os.makedirs(PREDICT_MINS, exist_ok=True)
earliest_date = df_ghost["GAME_DATE"].min().strftime("%Y%m%d")
out_file = os.path.join(PREDICT_MINS, f"next_game_mins_preds_{earliest_date}.csv")
df_ghost[["Player_ID", "PLAYER_NAME", "Team_Abv", "Opponent", "Home", "GAME_DATE", "MIN_PRED"]].to_csv(out_file, index=False)
