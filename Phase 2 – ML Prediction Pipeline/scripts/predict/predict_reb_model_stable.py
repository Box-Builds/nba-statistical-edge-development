#!/usr/bin/env python3
"""
Next-game rebound predictor (team-level next game version)
----------------------------------------------------------
...
print("📥 Loading trained REB model...")
with open(REB_MODEL_STABLE, "rb") as f:
    model = pickle.load(f)
...
df_ghost["REB_PRED"] = model.predict(X)

# ============================================================
# Save Output
# ============================================================
os.makedirs(PREDICT_REB_STABLE, exist_ok=True)
earliest_date = df_ghost["GAME_DATE"].min().strftime("%Y%m%d")
out_file = os.path.join(PREDICT_REB_STABLE, f"reb_preds_stable_{earliest_date}.csv")

df_ghost[[
    "Player_ID", "PLAYER_NAME", "Team_Abv", "Opponent", "Home", "GAME_DATE",
    "MIN_PRED_OOF", "REB_PRED"
