#!/usr/bin/env python3
"""
Next-game threes made predictor (team-level next game version)
----------------------------------------------------------
...
print("📥 Loading trained FG3M model...")
with open(FG3M_MODEL_STABLE, "rb") as f:
    model = pickle.load(f)
...
# Predict
# ============================================================
print("🏀 Predicting FG3M...")
df_ghost["FG3M_PRED"] = model.predict(X)

# ============================================================
# Save Output
# ============================================================
os.makedirs(PREDICT_FG3M_STABLE, exist_ok=True)
earliest_date = df_ghost["GAME_DATE"].min().strftime("%Y%m%d")
out_file = os.path.join(PREDICT_FG3M_STABLE, f"fg3m_preds_stable_{earliest_date}.csv")

df_ghost[[
    "Player_ID", "PLAYER_NAME", "Team_Abv", "Opponent", "Home", "GAME_DATE",
    "MIN_PRED_OOF", "FG3M_PRED"
