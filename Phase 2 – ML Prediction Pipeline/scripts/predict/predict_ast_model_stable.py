#!/usr/bin/env python3
"""
Next-game assists predictor (team-level stable version)
-------------------------------------------------------
...
RAW_STATS = ["AST", "PTS", "MIN", "TOV"]
ADV_STATS = ["USG%", "TS%", "eFG%", "PLUS_MINUS"]
...
print("📥 Loading trained AST model...")
with open(AST_MODEL_STABLE, "rb") as f:
    model = pickle.load(f)
...
for team_id, players in roster.items():
    ...
    for p in players:
        ...
        # Days rest
        row["Days_Rest"] = int((game_date - history["GAME_DATE"].max()).days) if not history.empty else 2
...
df_ghost["AST_PRED"] = model.predict(X)

# ============================================================
# Save Output
# ============================================================
os.makedirs(PREDICT_AST_STABLE, exist_ok=True)
earliest_date = df_ghost["GAME_DATE"].min().strftime("%Y%m%d")
out_file = os.path.join(PREDICT_AST_STABLE, f"ast_preds_stable_{earliest_date}.csv")

df_ghost[[
    "Player_ID", "PLAYER_NAME", "Team_Abv", "Opponent", "Home", "GAME_DATE",
    "MIN_PRED_OOF", "AST_PRED"
